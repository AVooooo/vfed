var dcys = {
	...
	'm3u8': {
		...
		'rows': [{
			...
			'name': '八戒云资源【已添加】',
			'rema': '顶部跑马灯水印',
			'apis': 'http://cj.bajiecaiji.com/inc/bjm3u8.php',
			'tips': '<span class="layui-badge layui-bg-red">国内分发</span>',
			...
			'name': '1886最快资源',
			'rema': '右下角短时出现水印',
			'apis': 'http://cj.1886zy.co/inc/zkm3u8.php',
			'tips': '<span class="layui-badge layui-bg-green">国外节点</span>',
			...
			'name': '永久云资源【已添加】',
			'rema': '顶部跑马灯水印',
			'apis': 'http://cj.yongjiuzyw.com/inc/s_yjm3u8.php',
			'tips': '<span class="layui-badge layui-bg-green">国外节点</span>',
			...
			'name': 'OK云资源【已添加】',
			'rema': '顶部跑马灯水印',
			'apis': 'https://cj.okzy.tv/inc/apickm3u8s_subname.php',
			'tips': '<span class="layui-badge layui-bg-green">国外节点</span>',
			...
			$.each(dcys, function(list, name) {
				...
				$.each(name.rows, function(nums, info) {
					...
					if(list == 'advs') {
						...
						html += '<td><a href="' + url1 + (list == 'down' ? '¶m=JmN0PTE' : '') + '">' + info.name + '【' + info.rema + '】</a></td>';
		...
		'init': function() {
			$(document).on('click', '.timming', function() {
				...
				layer.confirm('确认添加到定时任务吗？', {
					...
					$.post(vfed.tpls + 'asset/fed/create.php?id=tim&name=' + vfed.name, 'name=' + that.attr('data-name') + '&flag=' + that.attr('data-flag') + '¶m=' + encodeURIComponent(that.attr('data-param')), function(data) {
